			...
			const struct usb_ctrlrequest *ctrl)
{
	...
	print_debug("uf: gadget_setup: urb: %d, in: %d, out: %d\n",
			dev->setup_urb_queued, dev->setup_in_pending,
			dev->setup_out_pending);
	if (dev->setup_urb_queued) {
		ret = -EBUSY;
		goto out_unlock;
	}
	...
	if ((ctrl->bRequestType & USB_DIR_IN) && ctrl->wLength) {
		print_debug("uf: gadget_setup: setup_in_pending <= true\n");
		dev->setup_in_pending = true;
	} else {
		print_debug("uf: gadget_setup: setup_out_pending <= true\n");
...
static int fuzzer_ioctl_ep0_write(struct fuzzer_dev *dev, unsigned long value)
{
	...
	if (ret != 0) {
		print_debug("uf: fuzzer_ioctl_ep0_write: wait interrupted\n");
...
out_flags:
	...
	print_debug("uf: fuzzer_ioctl_ep0_write: setup_in_pending <= false\n");
out_unlock:
	spin_unlock_irqrestore(&dev->lock, flags);
	kfree(data);
out:
...
static int fuzzer_ioctl_ep0_read(struct fuzzer_dev *dev, unsigned long value)
{
	...
	ret = wait_for_completion_interruptible(&dev->setup_done);
	if (ret != 0) {
		print_debug("uf: fuzzer_ioctl_ep0_read: wait interrupted\n");
...
out_flags:
	...
	print_debug("uf: fuzzer_ioctl_ep0_read: setup_out_pending <= false\n");
out_unlock:
	spin_unlock_irqrestore(&dev->lock, flags);
	kfree(data);
out:
...
static int fuzzer_ioctl_ep_enable(struct fuzzer_dev *dev, unsigned long value)
{
	...
	print_debug("uf: fuzzer_ioctl_ep_enable: found free endpoint #%d\n", i);
	...
	gadget_for_each_ep(ep, dev->gadget) {
		...
		print_debug("uf: fuzzer_ioctl_ep_enable: usb_ep_enable = %d\n",
				ret);
		if (ret < 0)
			goto out_unlock;
		...
		ret = i;
		print_debug("uf: fuzzer_ioctl_ep_enable: enabled endpoint #%d\n", i);
		goto out_unlock;
	}
...
static int fuzzer_ioctl_ep_disable(struct fuzzer_dev *dev, unsigned long value)
{
	int ret = 0, i = value;
	unsigned long flags;
	const void *desc;

	print_debug("uf: fuzzer_ioctl_ep_disable\n");
	...
	print_debug("uf: fuzzer_ioctl_ep_disable: disabled endpoint #%d\n", i);

out_unlock:
	spin_unlock_irqrestore(&dev->lock, flags);
out:
	print_debug("uf: fuzzer_ioctl_ep_disable = %d\n", ret);
	return ret;
}
...
static int fuzzer_ioctl_ep_write(struct fuzzer_dev *dev, unsigned long value)
{
	...
	if (ret != 0) {
		print_debug("uf: fuzzer_ioctl_ep_write: wait interrupted\n");
...
static int fuzzer_ioctl_ep_read(struct fuzzer_dev *dev, unsigned long value)
{
	...
	print_debug("uf: fuzzer_ioctl_ep_read: urb queued, length: %d\n",
			io.length);

	ret = wait_for_completion_interruptible(&done);
	if (ret != 0) {
		print_debug("uf: fuzzer_ioctl_ep_read: wait interrupted\n");
	...
	spin_lock_irqsave(&dev->lock, flags);
	length = min(io.length, dev->eps[io.ep].req->actual);
	spin_unlock_irqrestore(&dev->lock, flags);

	print_debug("uf: fuzzer_ioctl_ep_read: urb completed, length: %d\n",
			length);
#if DEBUG
	print_hex_dump(KERN_ERR, "", DUMP_PREFIX_OFFSET,
			16, 1, data, length, false);
...
static long fuzzer_ioctl(struct file *fd, unsigned cmd, unsigned long value)
{
	...
	case USB_FUZZER_IOCTL_EP_ENABLE:
		...
		break;
	case USB_FUZZER_IOCTL_EP_DISABLE:
		ret = fuzzer_ioctl_ep_disable(dev, value);
		break;
