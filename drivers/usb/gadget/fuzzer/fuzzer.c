...
static void dev_put(struct fuzzer_dev *dev)
{
	...
	if (dev->req) {
		if (dev->setup_pending || dev->data_pending)
...
static void gadget_ep0_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	print_debug("uf: gadget_ep0_complete\n");

	/* Driver is being closed, usb_ep_dequeue() cancelled the request. */
	...
	/* Make sure we haven't failed nor started shutting down. */
	if (dev->state != STATE_DEV_RUNNING) {
		ret = -EBUSY;
		goto out_unlock;
	}

	// TODO: triggers on rpi zero
	BUG_ON(!dev->setup_pending && !dev->data_pending);

	if (dev->data_pending) {
		void *data;

		print_debug("uf: gadget_ep0_complete: pending data\n");
		// TODO: avoid double kmalloc in fuzzer_event_queue_add.
		data = kmalloc(sizeof(dev->ctrl) + dev->req->length,
					GFP_ATOMIC);
		if (!data) {
		...
		memcpy(data, &dev->ctrl, sizeof(dev->ctrl));
		memcpy(data + sizeof(dev->ctrl), dev->req->buf,
				dev->req->length);
		ret = fuzzer_event_queue_add(&dev->queue,
			USB_FUZZER_EVENT_CONTROL, sizeof(dev->ctrl) +
				dev->req->length, data);
		if (ret < 0) {
			kfree(data);
			dev->state = STATE_DEV_FAILED;
		...
		kfree(data);
		dev->data_pending = false;
		print_debug("uf: gadget_ep0_complete: data processed\n");
	} else {
...
out:
	print_debug("uf: gadget_ep0_complete = %d\n", ret);
}
			...
			const struct usb_ctrlrequest *ctrl)
{
	...
	if ((ctrl->bRequestType & USB_DIR_IN) || ctrl->wLength == 0) {
		print_debug("uf: gadget_setup: IN (or len == 0) request, len: %d\n",
				(int)ctrl->wLength);
		ret = fuzzer_event_queue_add(&dev->queue,
	...
	/*
	 * If this is an OUT control request, process DATA stage (receive
	 * data that comes with the request before queueing an event).
	 */
	print_debug("uf: gadget_setup: OUT request, len: %d\n",
				(int)ctrl->wLength);


	spin_lock_irqsave(&dev->lock, flags);

	data = kmalloc(ctrl->wLength, GFP_ATOMIC);
	if (!data) {
		ret = -ENOMEM;
		dev->state = STATE_DEV_FAILED;
		goto out_unlock;
	...
	dev->data_pending = true;

	print_debug("uf: gadget_setup: ep0 urb queued\n");
...
// TODO: forbid write without corresponding read?
static int fuzzer_ioctl_ep0_write(struct fuzzer_dev *dev, unsigned long value)
{
	...
	print_debug("uf: fuzzer_ioctl_ep0_write: ep0 urb queued\n");
